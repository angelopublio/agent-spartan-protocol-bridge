...
The producer does not run in the live repository. The runtime creates a realpath'd `0700` temporary workspace outside the repository, copies the admitted automatic write scope into it through descriptor-backed no-follow reads, adds a read-only `node_modules/` support tree so repository checks can run, and places a byte-identical `AGENTS.md` copy at the workspace root at mode `0444` so a working-directory launcher can resolve the repository's client context. Relative support symlinks are reconstructed only when their lexical target stays inside `node_modules/`. `node_modules/.cache/` is an always-applied disposable support scratch prefix. When `producer.scratch_prefixes` is absent from `spartan-bridge/config.yaml`, `dist/` is the default build scratch prefix; a non-empty declared list of relative trailing-slash prefixes replaces that build default, while the support prefix remains. Scratch output never merges back. A clean declaration strictly below a producer support root, such as `node_modules/.vite/`, is valid and is removed from the collapsed support digest as well as both producer-copy snapshots and merge classification. A resolved support-scratch path that is already a contained support symlink or another non-directory remains unchanged and read-only instead of being replaced or making preparation fail. A declaration equal to or covering a support root, overlapping the automatic implementation write scope, or covering the copied `AGENTS.md` refuses the chain as `config_invalid`; unrelated skipped roots and skipped descendants below a support root remain invalid. An inherited default that overlaps the write scope is instead dropped, so the write scope wins and that path remains product. The authority-file copy is represented as a regular file in the producer baseline; any producer content, mode, or deletion edit of it is refused during merge-back as `write_scope_violation`. This closes D-051's leftover-symlink escalation path: a symbolic alias can grant no write reach beyond a root the producer can already name directly. D-051 remains relevant only to the deliberate direct-write allowance for the child environment's existing `HOME` and `TMPDIR`.
